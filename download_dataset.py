...
def decompress_images(dataset_dir: str, keep_archive_after_decompress: bool):
    compressed_paths = [os.path.join(dataset_dir, path)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--keep_archive_after_decompress", action="store_true",
    ...
    urlretrieve(
        LABELS_AND_METADATA_URL, os.path.join(
            arguments.dataset_dir, os.path.basename(LABELS_AND_METADATA_URL)))
    urlretrieve(
        LABEL_RELATIONSHIP_URL, os.path.join(
            arguments.dataset_dir, os.path.basename(LABEL_RELATIONSHIP_URL)))
    # download and decompress all images
    for image_url in SMALL_IMAGE_URLS:
        logger.info("Downloading small image: {}".format(image_url))
        urlretrieve(image_url, os.path.join(
            arguments.dataset_dir, os.path.basename(image_url)))
